...
class ThinkingAnalyticsReporter {
  static Init() {
    ThinkDataLaunchReporter_1.ENABLE_THINKING_ANALYTICS &&
      ...
      EventSystem_1.EventSystem.Add(EventDefine_1.EEventName.LogOut, ThinkingAnalyticsReporter.Kvi),
      EventSystem_1.EventSystem.Add(
...
(exports.ThinkingAnalyticsReporter = ThinkingAnalyticsReporter),
  ((_a = ThinkingAnalyticsReporter).h9 = Stats_1.Stat.Create("ThinkingAnalyticsReporter.Track")),
  (ThinkingAnalyticsReporter.Wvi = () => {
    ...
    if (
      ...
      if (
        ...
        UE.ThinkingAnalytics.InitializeDefaultInsWithURL_Appid(
          ...
          Log_1.Log.CheckInfo() && Log_1.Log.Info("Login", 9, "数数上报实例已重新创建！");
      } else
        Log_1.Log.CheckError() && Log_1.Log.Error("Login", 9, `未找到 ${r} 对应的数数上报配置`);
    }
  }),
